...
export default function SplitTerminal({ credentials }: SplitTerminalProps) {
    ...
    // Track which pane IDs have rendered terminals (prevents remount)
    const renderedTerminalsRef = useRef<Set<string>>(new Set([rootPane.id]));
    ...
    const splitPane = useCallback((paneId: string, direction: 'horizontal' | 'vertical') => {
        ...
        console.log(`[splitPane] Splitting ${paneId} ${direction}, creating new pane: ${newPaneId}`);

        setRootPane(prev => {
            const transformPane = (pane: Pane): Pane => {
                if (!pane.children && pane.id === paneId) {
                    // Don't modify - just add to split list
                    // The pane will be wrapped by parent on next render
                    const newPane: Pane = {
                        id: newPaneId,
                    ...
                    // Keep pane as-is, wrap in container
                    return {
                        id: containerId,
                        splitDirection: direction,
                        children: [pane, newPane] as [Pane, Pane]
                    };
                }
